...
export default function EventDetail() {
  ...
  const [uploadType, setUploadType] = useState("csv"); // csv, excel, sheets
  ...
  const [activeTab, setActiveTab] = useState("participants"); // participants, template, certificates
        ...
        <div style={{ maxWidth: "1200px", margin: "0 auto" }}>
          <div className="card" style={{ marginBottom: "2rem" }}>
            <div style={{ marginBottom: "2rem", borderBottom: "1px solid var(--border-light)", paddingBottom: "1rem" }}>
              <h2 style={{ marginBottom: "0.5rem", fontSize: "1.5rem", fontWeight: "700" }}>Import Participants</h2>
              <p className="text-sm text-muted">Upload participant data from CSV, Excel files, or Google Sheets</p>
            </div>

            {/* Upload Type Selector */}
            <div style={{ marginBottom: "1.5rem" }}>
              <div style={{ display: "flex", gap: "0.5rem", padding: "0.25rem", background: "var(--bg-subtle)", borderRadius: "var(--radius-lg)" }}>
                <button
                  type="button"
                  ...
                  style={{
                    ...
                    cursor: "pointer",
                    transition: "all 0.2s",
                    background: uploadType === "csv" ? "white" : "transparent",
                    color: uploadType === "csv" ? "var(--primary)" : "var(--text-muted)",
                    boxShadow: uploadType === "csv" ? "var(--shadow-sm)" : "none"
                  }}
                >
                  CSV File
                </button>
                <button
                  ...
                  style={{
                    ...
                    cursor: "pointer",
                    transition: "all 0.2s",
                    background: uploadType === "excel" ? "white" : "transparent",
                    color: uploadType === "excel" ? "var(--primary)" : "var(--text-muted)",
                    boxShadow: uploadType === "excel" ? "var(--shadow-sm)" : "none"
                  }}
                >
                  Excel File
                </button>
                <button
                  ...
                  style={{
                    ...
                    cursor: "pointer",
                    transition: "all 0.2s",
                    background: uploadType === "sheets" ? "white" : "transparent",
                    color: uploadType === "sheets" ? "var(--primary)" : "var(--text-muted)",
                    boxShadow: uploadType === "sheets" ? "var(--shadow-sm)" : "none"
                  }}
                >
                  Google Sheets
                </button>
            ...
            <div className="alert alert-info" style={{ fontSize: "0.8125rem", padding: "0.75rem 1rem" }}>
              <strong>💡 Dynamic Column Support:</strong> The system automatically detects and stores ALL columns from your file. Common columns: name, email, roll_no, department, grade, etc.
            </div>

            <div style={{ marginTop: "2rem", paddingTop: "2rem", borderTop: "1px solid var(--border-light)" }}>
              <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: "1rem" }}>
                <h3 style={{ fontSize: "1.125rem", fontWeight: "700" }}>Participant List</h3>
                  ...
                  {participants.length === 0 ? (
                    ...
                    participants.map((p) => (
                      <tr key={p.id}>
                        ...
                        <td style={{ fontSize: "0.875rem" }}>{p.department || <span style={{ color: "var(--text-muted)" }}>-</span>}</td>
                      </tr>
                    ))
                  )}
                </tbody>
          ...
          <div className="card" style={{ marginBottom: "2rem" }}>
            <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: "2rem", paddingBottom: "1rem", borderBottom: "1px solid var(--border-light)" }}>
              <div>
                ...
                <p className="text-sm text-muted">Design and customize your certificate layout with drag-and-drop</p>
              </div>
              {selectedTemplate && (
                <div className="flex gap-2">
                  {isEditingLayout ? (
                    <>
                      <button className="btn btn-success" onClick={saveLayout} disabled={savingLayout}>
                        {savingLayout ? (
                          <>
                            <div className="loading-spinner" style={{ width: "16px", height: "16px", borderWidth: "2px", margin: 0 }}></div>
                            Saving...
                      ...
                      <button className="btn btn-secondary" onClick={cancelEditingLayout} disabled={savingLayout}>
                        Cancel
                      </button>
                    </>
                  ) : (
                    <button className="btn btn-primary" onClick={startEditingLayout}>
                      <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
            ...
            {!selectedTemplateId && !templatesLoading && (
               <div style={{ background: "var(--bg-subtle)", padding: "2rem", borderRadius: "var(--radius-lg)", border: "2px dashed var(--border-color)" }}>
                 <h3 style={{ marginBottom: "1rem" }}>Upload New Template</h3>
                 <form onSubmit={handleTemplateUpload}>
                   <div className="grid grid-cols-2 gap-4">
                     <div className="form-group">
                       <label>Template ID</label>
                       <input type="text" placeholder="e.g. classic-2025" value={templateForm.templateId} onChange={e => setTemplateForm({...templateForm, templateId: e.target.value})} required />
                     </div>
                     <div className="form-group">
                       <label>Template Name</label>
                       <input type="text" placeholder="e.g. Classic Certificate" value={templateForm.name} onChange={e => setTemplateForm({...templateForm, name: e.target.value})} required />
                     </div>
                   </div>
                   <div className="form-group">
                     <label>Image File</label>
                     <input type="file" accept="image/*" onChange={e => setTemplateFile(e.target.files?.[0])} required />
                   </div>
                   <button type="submit" className="btn btn-primary" disabled={templateUploading}>
      ...
      {activeTab === "certificates" && (
        <div style={{ maxWidth: "1200px", margin: "0 auto" }}>
          <div className="card">
            <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: "2rem", paddingBottom: "1rem", borderBottom: "1px solid var(--border-light)" }}>
              <div>
                <h2 style={{ marginBottom: "0.5rem", fontSize: "1.5rem", fontWeight: "700" }}>Generated Certificates</h2>
                <p className="text-sm text-muted">Manage and distribute certificates for this event</p>
              </div>
              <button 
            ...
            {certs.length === 0 ? (
              <div style={{ textAlign: "center", padding: "3rem", color: "var(--text-muted)" }}>
                ...
                <p style={{ fontSize: "0.9375rem" }}>Click "Generate All" to create certificates for all participants</p>
              </div>
            ) : (
...
function TemplatePreview({ template, apiBaseUrl, editable = false, layoutDraft, onLayoutChange }) {
  ...
  useEffect(() => {
    ...
    const handleMove = (event) => {
      ...
      // Use actual certificate dimensions (1200x800) for more precise positioning
      ...
      // Store as pixel values (backend will handle conversion)
  ...
  // Dynamically get all text fields from layout (excluding qr)
